...
// and on the Arduino PID and PID AutoTune Libraries 
// by Brett Beauregard
//------------------------------------------------------------------
...
// These #define makes it easy to set the backlight on
#define ON 0x1
...
void Off()
{
   ...
   while(!(buttons & (BUTTON_RIGHT)))
   {
      ...
      // Update the display with the meassured temperature to allow monitoring of the cool down 
      Input = getTempC();
      lcd.setCursor(0, 1);
      lcd.print(F("  Temp: "));
      lcd.print(Input,1);
...
// ************************************************
// Read temperature from PTC sensor
// ************************************************
float getTempC(void) {
  uint8_t i;
  float average;
